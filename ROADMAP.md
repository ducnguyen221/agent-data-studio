...
> chảy về **thư mục dự án do USER CHỈ ĐỊNH ngoài repo** (mặc định `~/powerbi-project`, user đổi được).
...
- [x] Tool MCP `distill_report_design(report_path, out_dir)` — quét toàn bộ `*.Report`:
...
- [x] Tư vấn lưu template ĐỒNG BỘ: KHÔNG commit cả .pbip vào repo (nặng + lộ nghiệp vụ);
  chuẩn = kit per-page sanitize → `report-templates/` repo, còn **full project + kit thô → thư mục dự án**.
