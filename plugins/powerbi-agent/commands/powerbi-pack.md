...
2. Giao agent **`powerbi-knowledge-curator`** (subagent) với prompt: đường dẫn root + slug dự án (nếu có). Host không hỗ trợ subagent → tự thực hiện đúng quy trình trong định nghĩa agent (`plugins/powerbi-agent/agents/powerbi-knowledge-curator.md`): đọc INDEX/TIMELINE/knowledge hiện có → đọc dự án → rút bài học TÁI DÙNG (ngưỡng cao) → DEDUP (cập nhật file cũ) → ghi theo chuẩn Why/How-to-apply → cập nhật INDEX + TIMELINE.
